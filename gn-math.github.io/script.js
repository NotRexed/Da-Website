...
const zonesurls = [
    "https://cdn.jsdelivr.net/gh/NotRexed/assets/zone.json"
];
      ...
      let sha;
        ...
        if (sharesponse && sharesponse.status === 200) {
          try {
            ...
            sha = shajson[0]['sha'];
            if (sha) {
                zonesURL = `https://cdn.jsdelivr.net/gh/NotRexed/assets/zone.json`;
            }
          } catch (error) {
            try {
...
settings.addEventListener('click', () => {
    ...
    const popupBody = document.getElementById('popupBody');
    popupBody.innerHTML = `
    <button id="settings-button" onclick="darkMode()">Toggle Color Mode</button>
    <br><br>
